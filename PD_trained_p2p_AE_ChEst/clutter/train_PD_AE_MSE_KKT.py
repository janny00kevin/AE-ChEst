...
def train(size, hidden_sizes, lr, num_epochs, batch_size, num_minibatch, cuda, snr, channel, eps_fidel):
    ...
    def KKT_termination(logits_net, constraint_loss, lambda_val, epsilon):
        ...
        return gradient_norm <= epsilon and constraint_loss <= 0 and lambda_val.real >= 0 and lambda_val*constraint_loss <= epsilon
    ...
    ## initialize lambda   
    lamb = torch.tensor([10.0], requires_grad=True, device=device)    ####################################################
    lr_l = 1e-1 #5e-5                                             
    epsilon = 1e-16
    ...
    for i in range(num_epochs):
        ...
        for j in range(num_minibatch):
            ...
            with torch.no_grad():
                lamb = lamb + lr_l*torch.exp(-0.005*torch.tensor(itr)) * constraint_loss.detach()  ####################################################
                lamb[lamb < 0] = 0
    ...
    save_path = os.path.join(script_dir, 'trained_model', '%.3f_eps%.0e_%s_AE_lr%.0e_%s_ep%s' 
