...
plt.plot(SNR_dB, NMSE_2.to('cpu'), label='parametric PD AE $\\varepsilon=15$', linewidth=1, linestyle='-', marker='x', color="tab:pink")  ###
...
data = {
    ...
    "AE 1": NMSE_1.to('cpu'), "AE 2": NMSE_2.to('cpu'), "MLP": NMSE_3.to('cpu'),
    "channelNet": NMSE_6.to('cpu'), "ISTANet": NMSE_9.to('cpu')
}
df = pd.DataFrame(data)
save_path = os.path.join(script_dir, 'ChEsts_testing_performance', '%s_AE_vs_CN_IN__.pdf'%(channel)) #  %(IRS_coef_type)
