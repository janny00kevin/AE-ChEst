...
def train(size, hidden_sizes, lr, num_epochs, batch_size, num_minibatch, cuda, snr, channel, eps_fidel):
    ...
    ## initialize lambda   
    lamb = torch.ones(batch_size, requires_grad=False, device=device)
    lr_l = 1e-2 #5e-5                                                    ###
    epsilon = 1e-16
    ...
    for i in range(num_epochs):
        ...
        for j in range(num_minibatch):
            ...
            constraint_loss = torch.norm(tau_y - logits, p=2, dim=1)**2 - eps_fidel
            # norm = torch.norm(turnCplx(tau_h) - D.matmul(tau_tbh_cplx.T).T, dim=1)**2

            ## dual variable lambda update
            with torch.no_grad():
                lamb = lamb + lr_l/itr * constraint_loss.detach()
                lamb[lamb < 0] = 0

            # lamb_dt = lamb.detach().clone()

            ## primal variable theta update
            lagrangian = l1_loss + torch.sum(lamb*(constraint_loss))
        ...
        # if termination(logits_net, priFeasibility, lamb_rec[itr-1], epsilon) or :
    ...
    save_path = os.path.join(script_dir, 'trained_model', '%.3f__%sAE_lr%.0e_%s_ep%s' 
            %(best_loss, channel, lr, [2*n_R*T]+hidden_sizes+[2*n_R*n_T], train_epochs))
    lossPlot()
    parametersSave()
...
if __name__ == '__main__':
    ...
    parser.add_argument('-mbs', type=int, default=9000)  # 10k length of trajectory = size of mini-batch
    ...
    parser.add_argument('-eps', type=float, default=1e-5)  # epsilon for termination
