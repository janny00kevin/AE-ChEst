...
def train(size, hidden_sizes, lr, num_epochs, batch_size, num_minibatch, cuda, snr, channel, eps_fidel):
    ...
    ## initialize lambda   
    lamb = torch.tensor([10000.0], requires_grad=True, device=device)       ###
    lr_l = 1e-1 #5e-5                                             
    epsilon = 1e-16
    ...
    scheduler = ReduceLROnPlateau(optimizer_pri, mode='min', patience=20, factor=0.5, threshold=1e-3)  ####################################################
    ...
    for i in range(num_epochs):
        ...
        for j in range(num_minibatch):
            ...
            constraint_loss = torch.mean(torch.norm(tau_y - logits, p=2, dim=1)**2) - eps_fidel
            # norm = torch.norm(turnCplx(tau_h) - D.matmul(tau_tbh_cplx.T).T, dim=1)**2

            ## dual variable lambda update
            with torch.no_grad():
                # lamb = lamb + lr_l * constraint_loss.detach()  ####################################################
                lamb[lamb < 0] = 0
            ...
            pbar.set_description('tNMSE:%s, vNMSE:%s, -4:%s, 10:%s, l:%s' 
...
if __name__ == '__main__':
    ...
    parser.add_argument('-eps', type=float, default=1e-100)  # 5e1    ####################################################
