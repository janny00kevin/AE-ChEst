...
if dataset == 'InF':
    load_training_file = './data/A_Training_Dataset_Eigen_InF25_TisnT_neg10and15.mat'
...
def update_rho(norm_r, norm_s, rho, mu=10, tau_inc=2, tau_dec=2):
    ratio = norm_r / norm_s
    if norm_r > mu*norm_s: return rho*tau_inc
    elif norm_s > mu*norm_r: return rho/tau_dec
    else: return rho

def convergence_criterion(norm_r, norm_s, z, h_hat, grad, n_param, ep_abs=1e-3, ep_rel=1e-3):
    p,b = z.shape
    ...
    for e in range(epochs):
        ...
        for batch in tqdm(dataloader_train):
            ...
            while k < k_max:
                ...
                j_used = update_theta(network, receivedSignal, x_old, u, rho, j_max=1000)
                ...
                grad = torch.autograd.grad(temp, network.parameters(), grad_outputs=torch.ones_like(temp), retain_graph=True)
                converge, ep_prim, ep_dual = convergence_criterion(norm_r, norm_s, x_new_, h_hat_, grad, n_param, ep_abs=3e-3, ep_rel=3e-3)
                ...
                if k % 10 == 0: plot_01(totalr, totals, totalrho, fig_name_02)  # plot every 10 iterations
