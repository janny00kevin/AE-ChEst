...
def convergence_criterion(norm_r, norm_s, z, h_hat, temp, n_param, ep_abs=1e-3, ep_rel=1e-3):
    ...
    return all_primal_converge and all_dual_converge, max(primal_convergence), max(dual_convergence)
    ...
    for e in range(epochs):
        ...
        for batch in tqdm(dataloader_train):
            ...
            while k < k_max:
                ...
                converge, normalized_prim, normalized_dual = convergence_criterion(norm_r, norm_s, x_new_, h_hat_, temp, n_param, ep_abs=3e-3, ep_rel=3e-3)
