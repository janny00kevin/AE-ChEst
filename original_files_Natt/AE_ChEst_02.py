...
if dataset == 'InF':
    ...
    # load_testing_file = './data/UMa_10k.mat'
    scaling = 1e5
    lambda_sparsity = 1e8  # Sparsity penalty weight

model_name = 'model/model_parameters_AE_ChEst_02_UMa_01.pth'
fig_name = "fig/training_curve_02_UMa_01.png"
fig_name_02 = "fig/scratch02_UMa_01.png"
device = torch.device("cuda:1" if torch.cuda.is_available() else "cpu")
n_layer = 10
epsilon = 1e-8
rho = 200            # [Training] weighting of reconstruction error in updating model
k_max = 20
j_max = 1
epochs = 3000
max_iter = 1000     # [Training] for convergence of x and u
max_count = 300     # [Testing] for ISTA algorithm
scaling_ISTA = 1
t = [0.001]
lamb = [10,20,30]
test_network = 1
...
def plot_01(totalr, totals, totalrho, fig_name):
    plt.clf()
    plt.plot(totalr[1:], label='primal residual', color='blue')
    plt.plot(totals[1:], label='dual residual', color='red')
    plt.plot(totalrho[1:], label='rho', color='green')
...
def get_residual_norm(h_hat, z_kk, grad):
    r = z_kk - h_hat.detach()
    norm_r = torch.norm(r, 'fro')

    norm_s_sq = 0
    for j in range(len(grad)):
        norm_s_sq += torch.norm(grad[j])**2
    norm_s = torch.sqrt(norm_s_sq)

    return norm_r, norm_s

def update_rho(norm_r, norm_s, rho):
    mu = 10
    tau = 2
    if norm_r > mu*norm_s: return rho*tau, norm_r, norm_s
    elif norm_s > mu*norm_r: return rho/tau, norm_r, norm_s
    else: return rho, norm_r, norm_s

def convergence_criterion(norm_r, norm_s, z, h_hat, grad, n_param):
    ep_abs = 1e-3
    ep_rel = 1e-3
    p = z.shape[0]
    ...
    ep_prim = np.sqrt(p)*ep_abs*batch_size + ep_rel*max(torch.norm(z, 2), torch.norm(h_hat, 2))
    ep_dual = np.sqrt(n_param)*ep_abs*batch_size + ep_rel*norm_grad
    return norm_r <= ep_prim and norm_s <= ep_dual, ep_prim, ep_dual
...
class SparseAutoencoder(nn.Module):
    def __init__(self, input_dim, n_layer):
        ...
        self.lamb = nn.Parameter(torch.tensor(1, dtype=torch.float32))
...
if mode == 'train':
    ...
    optimizer = torch.optim.Adam(network.parameters(), lr=1e-4)      # lr=1e-5 (slow)
    scheduler = ReduceLROnPlateau(optimizer, mode='min', patience=2, factor=0.5, verbose=True)   # patience=2, factor=0.1
    min_lr = 5e-7
    lossListTrain = []
    lossListTest = []
    ...
    for e in range(epochs):
        ...
        for batch in tqdm(dataloader_train):
            ...
            loss_e = torch.tensor(999999)
              
            x_new = torch.zeros((batch_size, len_h*2), dtype=torch.float).to(device)
            u = torch.zeros((batch_size, len_h*2), dtype=torch.float).to(device)
            k = 0
            converge = False
            while not converge and k < k_max:
                k += 1
                old_loss = loss_e.item()
                x_old = x_new
                ...
                grad = torch.autograd.grad(temp, network.parameters(), grad_outputs=torch.ones_like(temp), retain_graph=True)
                norm_r, norm_s = get_residual_norm(h_hat_, x_new_, grad)
                del temp, grad
                ...
                grad = torch.autograd.grad(temp, network.parameters(), grad_outputs=torch.ones_like(temp), retain_graph=True)
                converge, ep_prim, ep_dual = convergence_criterion(norm_r, norm_s, x_new_, h_hat_, grad, n_param)
                del temp, grad
                torch.cuda.empty_cache()

                totalr.append((norm_r / ep_prim).item())
                totals.append((norm_s / ep_dual).item())
                totalrho.append(rho)

                loss_e = nn.MSELoss()(h_hat, groundChannel)
                e_.append(loss_e.detach().item())
                if converge: break
                else: rho, norm_r, norm_s = update_rho(norm_r, norm_s, rho)

                if k % 10 == 0: plot_01(totalr, totals, totalrho, fig_name_02)
                #     plt.clf()
                #     plt.plot(totalr[1:], label='primal residual', color='blue')
                #     plt.plot(totals[1:], label='dual residual', color='red')
                #     plt.plot(totalrho[1:], label='rho', color='green')
        ...
        print("  Epoch {:d}\t| Training loss: {:.4e}  | Testing loss: {:.4e} --- ratio: {:.3f} | Weight on Spars: {:.2f}% ".format(e+1, avgTrainLoss, avgTestLoss, avgTrainLoss/avgTestLoss, sparse_loss/loss_all*100))
        # print("     Fidelity loss: {:.4e}     Sparsity loss: {:.4e}".format(fidelity_loss, sparse_loss))
        plt.clf()  # Clear previous plot
        plt.plot(lossListTrain, label='Training Loss', color='blue')
        plt.plot(lossListTest, label='Testing Loss', color='red')
        plt.xlabel('Epoch')
        plt.ylabel('Loss')
    ...
    MC = 3000
    len_h = int(len_h_2/2)
    network_1st = SparseAutoencoder(len_h_2, n_layer).to(device)
